...
def detections(batch, model, max_det, batch_size, device):
    ...
    for i in range(batch_size):
        ...
        img = cv2.imread(f"D:\\FramesDataset\\{batch['game_id'][0]}\\tick_{tick}.jpg")
...
def evaluate(val_loader, radar_encoder, yolo, temporal_model, mse_loss, bce_loss, device, max_det, batch_size):
    radar_encoder.eval()
    yolo.eval()
    temporal_model.eval()
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader, desc="Validation", leave=False):
            ...
            gt_mouse = batch["labels_mouse"].to(device)
            gt_keys = batch["labels_keys"].to(device)
            ...
            total_loss += (loss_mouse + loss_keys).item()
    ...
    print(f"Validation — Loss: {avg_loss:.4f} | MSE(mouse): {mse_mouse:.4f} | Precision(keys): {precision:.4f} | Recall(keys): {recall:.4f}")
...
def train():
    ...
    yolo = DetectionModel(cfg=r"C:\Users\misas\CS2_NN\src\yolo11n.yaml").to(device)
    yolo = load_pretrained_weights(yolo, r"C:\Users\misas\CS2_NN\yolo11n.pt").to(device).eval()
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        avg_val_loss = evaluate(val_loader, radar_encoder, yolo, temporal_model, mse_loss, bce_loss, device, MAX_DET, BATCH_SIZE)
        ...
        if avg_val_loss < best_val_loss:
            ...
            }, os.path.join(CKPT_DIR, f"best_epoch_{epoch+1}.pth"))
